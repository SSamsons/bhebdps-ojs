describe("About Higher Order Functions (about_higher_order_functions.js)", function () {
  ...
  it("should use filter to return array items that meet a criteria", function () {
    ...
    expect(odd).toEqual([1, 3]); // Фильтруем нечетные числа
    expect(odd.length).toBe(2); // В массиве 2 элемента
  ...
  it("should use 'forEach' for simple iteration", function () {
    ...
    let isEven = function (item) {
      msg += (item % 2) === 0; // true/false будет добавлено в строку
    };

    numbers.forEach(isEven);

    expect(msg).toEqual("falsetruefalse"); // 1 -> false, 2 -> true, 3 -> false
    expect(numbers).toEqual([1, 2, 3]); // Исходный массив не изменился
  });
  ...
  it("should use 'every' to test whether all items pass condition", function () {
    ...
    expect(onlyEven.every(isEven)).toBe(true); // Все элементы четные
    expect(mixedBag.every(isEven)).toBe(false); // Не все элементы четные
  });
  ...
  it("should use 'some' to test if any items passes condition" , function () {
    ...
    expect(onlyEven.some(isEven)).toBe(true); // Хотя бы один элемент четный
    expect(mixedBag.some(isEven)).toBe(true); // Хотя бы один элемент четный
  });

  it("should use flatten to make nested arrays easy to work with", function() {
      expect([ [1, 2], [3, 4] ].flat()).toEqual([1, 2, 3, 4]); // Объединяем вложенные массивы
  });
  ...
  it("should use flat() ... reduce() to use multiple higher order functions", function() {
      ...
      // flat() -> [0, 1, 2], map() -> [1, 2, 3], reduce() -> 1 + 2 + 3 = 6
